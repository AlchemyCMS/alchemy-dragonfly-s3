...
RSpec.describe Alchemy::PictureThumb::Uid do
  ...
  let(:picture) { FactoryGirl.build_stubbed(:alchemy_picture, image_file: image) }
  ...
  context "with format options" do
    ...
    it "uses this as extension" do
      is_expected.to eq "pictures/#{picture.id}/12345/image.jpg"
    end
  end

  context "with non word characters in filename" do
    let(:picture) { FactoryGirl.build_stubbed(:alchemy_picture, image_file: image, image_file_name: "The +*&image).png") }
  ...
  context "with no image_file_name" do
    let(:picture) { FactoryGirl.build_stubbed(:alchemy_picture, image_file: image, image_file_name: nil) }
