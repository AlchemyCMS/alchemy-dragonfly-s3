...
RSpec.describe Alchemy::PictureThumb::Uid do
  ...
  let(:picture) { FactoryBot.build_stubbed(:alchemy_picture, image_file: image) }
  ...
  context "with non word characters in filename" do
    let(:picture) { FactoryBot.build_stubbed(:alchemy_picture, image_file: image, image_file_name: "The +*&image).png") }
  ...
  context "with no image_file_name" do
    let(:picture) { FactoryBot.build_stubbed(:alchemy_picture, image_file: image, image_file_name: nil) }
